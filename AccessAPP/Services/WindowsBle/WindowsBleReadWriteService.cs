...
public class WindowsBleReadWriteService : IBleReadWriteService
{
    ...
    {
        // Retry once if the session is concurrently disposed (race with ConnectToBleDevice /
        // DisconnectFromBleDevice on the same MAC). On ObjectDisposedException we invalidate
        // the stale caches so the retry fetches fresh device/characteristic references.
        for (int attempt = 0; attempt < 2; attempt++)
        {
            ...
            {
                // A concurrent DisposeDevice call invalidated the cached device/characteristic.
                // Clear stale caches so the retry obtains fresh references.
                _logger.LogWarning("WinBLE Write: {Mac} ObjectDisposed on attempt 1 — invalidating caches and retrying (handle {Handle})", macAddress, handle);
                WindowsBleHelpers.InvalidateCharCache(macAddress);
                WindowsBleHelpers.InvalidateModeCache(macAddress);
            }
            catch (Exception ex)
            {
